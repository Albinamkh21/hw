...
var Model = {
   ...
   .then(function (photos) {
    ...
    getPhotos2:  function (aid) {
            return new Promise(function(resolve, reject) {
            db.transaction(function (tx) {
                tx.executeSql('SELECT pid, src, text, aid, commentsCount,likesCount, repostsCount FROM photos  p where p.aid = ? order by  pdate  ', [aid], function (tx, res) {
                   resolve(res.rows);
                });
            });
